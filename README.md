...
параметр ```secret```, найденный на шаге 4 передается в строке запроса (query string), post-параметр ```special``` передается со значением ```data-value```, найденным на шаге 5

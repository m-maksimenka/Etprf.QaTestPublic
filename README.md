...
6. Сделайте POST запрос с помощью утилиты Postman. Вам нужно отправить запрос на адрес ```http://funkyb.ru/api/stage2```
параметр ```secret```, найденный на шаге 4 передается в строке запроса (query string), post-параметр ```special``` передается со значением ```data-value``` в формате JSON, найденным на шаге 5
